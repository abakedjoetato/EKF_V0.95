...
public class DeadsideCsvParser {
    ...
    // Flag to indicate if we are processing historical data
    private boolean processingHistoricalData = false;
    ...
    /**
     * Set whether the parser is currently processing historical data
     * When processing historical data, we should still update player stats
     * but suppress Discord embed notifications
     * 
     * @param processingHistoricalData true if processing historical data, false otherwise
     */
    public void setProcessingHistoricalData(boolean processingHistoricalData) {
        this.processingHistoricalData = processingHistoricalData;
        logger.info("Historical data processing mode set to: {}", processingHistoricalData);
    }
    
    /**
     * Check if the parser is currently processing historical data
     * 
     * @return true if processing historical data, false otherwise
     */
    public boolean isProcessingHistoricalData() {
        return processingHistoricalData;
    }
    
    /**
     * Process death log content directly from a string
     * Used primarily for historical processing
     * 
    ...
    private void sendPlayerKillKillfeed(GameServer server, String timestamp, String victim, String victimId,
                                       String killer, String killerId, String weapon, int distance) {
        try {
            // Skip sending notifications if we're processing historical data
            if (processingHistoricalData) {
                logger.debug("Historical processing: skipping killfeed notification for {} killed {} with {}", 
    ...
    private void sendRelocationKillfeed(GameServer server, String timestamp, String victim, String victimId) {
        ...
        // During historical processing, just log with appropriate level
        if (processingHistoricalData) {
            logger.debug("Historical processing: player {} relocated at {}", victim, timestamp);
            return;
        }
    ...
    private void sendSuicideKillfeed(GameServer server, String timestamp, String victim, String victimId, String cause) {
        try {
            // Skip sending notifications if we're processing historical data
            if (processingHistoricalData) {
                logger.debug("Historical processing: skipping suicide notification for {} with cause {}", 
                        victim, cause);
                return;
            }
